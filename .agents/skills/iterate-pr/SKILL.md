...
!`node "${CLAUDE_PLUGIN_ROOT:-.agents}/skills/unresolved-pr-comments/unresolvedPrComments.ts" 2>/dev/null`
...
> 6. **Check Comments**: Run `node "${CLAUDE_PLUGIN_ROOT:-.agents}/skills/unresolved-pr-comments/unresolvedPrComments.ts"` and parse the JSON output
...
2. If no commits were made this iteration:
   - Get unresolved comments by running: `node "${CLAUDE_PLUGIN_ROOT:-.agents}/skills/unresolved-pr-comments/unresolvedPrComments.ts"`
