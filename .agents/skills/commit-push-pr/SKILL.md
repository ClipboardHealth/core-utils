...
6. Check for an existing PR with `gh pr view`.
   ...
   - PR exists: refresh the body via `gh pr edit --body` whenever either of the following is stale, preserving everything else (including the `<!-- commit-push-pr:created v1 -->` sentinel). Then report the URL.
     ...
     - **`Agent session ID:` line(s)**: the line to add is `Agent session ID: <output of step 5>` (e.g. `Agent session ID: claude-code c8fb7000-...`) — same format as the "No PR" branch above. If that exact line is already present in the body, do nothing. Otherwise append it on its own line, just before the `<!-- commit-push-pr:created v1 -->` sentinel (or at the end of the body if no sentinel is present). Never modify or remove existing `Agent session ID:` lines — every contributing session's line is preserved so the agent history isn't lost. If step 5 produced no output, do nothing here.
