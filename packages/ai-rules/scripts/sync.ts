...
async function sync() {
  ...
  // Run formatter outside the try/catch so errors propagate with non-zero exit codes
  const filesToFormat = [
    path.join(PATHS.projectRoot, FILES.agents),
    path.join(PATHS.projectRoot, ".rules"),
  ];
  await runAvailableFormatter(PATHS.projectRoot, filesToFormat);
}
