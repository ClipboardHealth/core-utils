...
- Use `discriminatedUnionWithFallback(discriminator, values, variants)` for discriminated unions whose variant set can grow — not `z.discriminatedUnion()` (a new variant breaks old mobile clients) or a hand-rolled `z.union()`. One call returns a `{ request, response }` pair: `request` rejects an unknown discriminator and unknown keys, `response` collapses an unknown discriminator to `ENUM_FALLBACK` and strips unknown keys, and both still reject a known variant with a missing field or an invalid value. The value tuple and the variant record must line up exactly and each variant must declare `z.literal` of its own key, so a known variant can never silently read as `ENUM_FALLBACK`
