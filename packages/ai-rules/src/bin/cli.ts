...
function updatePackageJson(profile: Profile, options: { postinstall: boolean }): void {
  ...
  // Add sync script
  pkg.scripts['sync-ai-rules'] = `npx @clipboard-health/ai-rules apply --profile=${profile}`;
...
function updateGitignore(): void {
  ...
  const aiRulesBlock = `
# AI Agent Rules (auto-generated by @clipboard-health/ai-rules)
AGENTS.md
CLAUDE.md
.cursor/
`;

  fs.appendFileSync(gitignorePath, aiRulesBlock);
}
...
function applyRules(categories: readonly string[], dryRun = false): void {
  ...
  try {
    ...
    if (dryRun) {
      ...
      console.log('  3. Add to .gitignore if desired (optional)');
    }
  } catch (error) {
...
program
  ...
  .addOption(
    new Option('--profile <profile>', 'Use specific profile (non-interactive)')
      .choices(['frontend', 'backend', 'fullstack', 'common'])
  )
  .action(async (options: { profile?: string }) => {
    ...
    try {
      ...
      // 4. Ask about .gitignore (interactive only)
      let shouldUpdateGitignore = false;
      if (isInteractive && fs.existsSync('.gitignore')) {
        shouldUpdateGitignore = await confirm({
      ...
      if (shouldUpdateGitignore) {
        ...
        console.log('   Added: AGENTS.md, CLAUDE.md, .cursor/');
      }
...
program
  ...
  .addOption(
    new Option('-p, --profile <profile>', 'Apply a predefined profile')
      .choices(['frontend', 'backend', 'fullstack', 'common'])
  )
  .addOption(
    new Option('-r, --ruleset <rulesets>', 'Apply specific rulesets (comma-separated)')
  )
  .addOption(
