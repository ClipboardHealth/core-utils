...
describe("GET /users", () => {
  ...
  it("defaults page size if no query string", () =>
    request(app.getHttpServer()).get("/users").expect(200).expect(defaultQuery));

  it("parses page size", () =>
    request(app.getHttpServer())
      .get("/users?page[size]=1")
      .expect(200)
      .expect({ page: { size: 1 } }));

  it("parses page cursor", () =>
    request(app.getHttpServer())
      .get("/users?page[cursor]=abc")
      .expect(200)
      .expect({ page: { size: 20, cursor: "abc" } }));

  it("parses single field value", () =>
    request(app.getHttpServer())
      .get("/users?fields[user]=age")
      .expect(200)
      .expect({ ...defaultQuery, fields: { user: ["age"] } }));

  it("parses multiple field values", () =>
    request(app.getHttpServer())
      .get("/users?fields[user]=age,dateOfBirth")
      .expect(200)
      .expect({ ...defaultQuery, fields: { user: ["age", "dateOfBirth"] } }));

  it("parses multiple api type fields", () =>
    request(app.getHttpServer())
      .get("/users?fields[user]=age,dateOfBirth&fields[article]=title")
      .expect(200)
  ...
  it("fails if page size above maximum", async () => {
    const response = await request(app.getHttpServer()).get("/users?page[size]=201");
  ...
  it("fails if page cursor is empty", async () => {
    const response = await request(app.getHttpServer()).get("/users?page[cursor]=");
  ...
  it("fails if invalid field", async () => {
    const response = await request(app.getHttpServer()).get("/users?fields[user]=height");
  ...
  it("fails if invalid number filter", async () => {
    const response = await request(app.getHttpServer()).get("/users?filter[age]=abc");
  ...
  it("fails if invalid filter type", async () => {
    const response = await request(app.getHttpServer()).get("/users?filter[age][lt]=1");
  ...
  it("handles multiple filters using '&'", () =>
    request(app.getHttpServer())
      .get("/users?filter[age]=10&filter[age]=20")
      .expect(200)
      .expect({ ...defaultQuery, filter: { age: { eq: [10, 20] } } }));

  it("handles multiple filters using '&' with an additional filter type", () =>
    request(app.getHttpServer())
      .get("/users?filter[age]=10&filter[age]=20&filter[age][gt]=30")
      .expect(200)
      .expect({ ...defaultQuery, filter: { age: { eq: [10, 20], gt: [30] } } }));

  it("fails if invalid boolean filter", async () => {
    const response = await request(app.getHttpServer()).get("/users?filter[isActive]=1");
  ...
  it("fails if invalid date filter", async () => {
    const response = await request(app.getHttpServer()).get(
      "/users?filter[dateOfBirth]=16:06:12.125",
    );
