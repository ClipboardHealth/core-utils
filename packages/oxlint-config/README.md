...
| `style`                             | `off`   | Idiomatic/consistency rules with no bug-prevention value. The few worth keeping are opted back in explicitly (e.g. `curly`, `no-else-return`, `no-new-func`, `no-template-curly-in-string`). |
