...
**Cause:** `hex project import` can only *update* an existing App canvas. It
...
6. User checked the *original* Payments dashboard's App view — blank too. The
...
This is the same `{{ var }}` slot gotcha 4 says *not* to quote — the parens
...
- For a *single* date input (`useDateRange: false`), the variable IS bound to
