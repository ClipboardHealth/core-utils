...
3. Before creating side effects, search STAFF-1818 comments, Linear tickets, and GitHub PRs for `<!-- flaky-critic: b8-prior-monitoring-action run=2026-07-16--2026-07-30 -->`. Reuse the linked action when present.
4. When a new action is required, the claim winner posts `<!-- flaky-critic: b8-prior-monitoring-action-started run=2026-07-16--2026-07-30 owner=<claim comment ID> lease-until=<ISO timestamp> -->` with a four-hour lease, then re-reads STAFF-1818. The earliest unexpired action lease wins; later action starters stop. Renew the lease before invoking `cb-work` and at least every three hours while work continues.
5. Revalidate the action lease and repeat the action search immediately before any PR or ticket creation. The `cb-work` request must carry this same requirement for its shipping step: if ownership changed or an action marker appeared, abort creation and reuse the existing link. A fallback ticket creator performs the same check immediately before its create call. Any new PR or ticket must carry the action marker, an exact `STAFF-1818 monitoring` title suffix, and the deterministic run key. A takeover is allowed only after the prior action lease expires with no action marker.
