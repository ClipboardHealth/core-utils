...
The skill uses two sentinels. Each is a **visible footer line** — a 🤖 mark plus a machine-readable token inside an inline `<code>` span, wrapped in `<sub>` so it renders small and grey. Human reviewers can see at a glance that the reply came from an agent (replies post through the user's own account, so without this they look hand-typed), while re-runs still dedupe off the embedded token.

**Addressed sentinel**: `<sub>🤖 <code>babysit-pr:addressed v1 core@3.4.1</code></sub>`. The `core@<X.Y.Z>` suffix records which plugin version produced the reply. Appended on its own line at the end of every reply the skill posts (both thread replies and the review-body summary). This is how the skill knows, on re-runs, which threads and automated review-body comments it already handled. Dedupe matches by the version-agnostic substring `babysit-pr:addressed v1` followed by a single space — present in both this footer and the legacy hidden-comment form (`<!-- babysit-pr:addressed v1 ... -->`), so sentinels left by pre-versioning and pre-visibility plugin versions are still recognized. Grep `babysit-pr:addressed v1` to find sentinels regardless of version or wrapper; grep `babysit-pr:addressed v1 core@3.4.1` to find ones from a specific version.
