...
function getMessageForStatus(status: Status, errorMessage?: string): string | undefined {
  switch (status) {
    ...
    case STATUS.notInstalled: {
      return `${PACKAGE_NAME} is not installed. Ask the user if they'd like to install it. If approved, fetch ${README_URL} and follow the instructions. Ask the user: "Which profile to use (common, frontend, backend, datamodeling)? See ${README_URL} for details."`;
    }
